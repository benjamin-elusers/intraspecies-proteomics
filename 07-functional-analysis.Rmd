...
kk2 = clusterProfiler::enrichKEGG(genes_dfe_uni,organism = 'sce', keyType = 'uniprot', pvalueCutoff = 0.05, universe =genes_proteomics_uni)
dotplot(kk2)
heatplot(kk2, foldChange=genes_dfe_list_uni)

go2 = clusterProfiler::enrichGO(genes_dfe,universe =genes_proteomics, 
                                OrgDb = org.Sc.sgd.db, keyType = 'ORF',
                                ont = 'ALL', pool = F,pvalueCutoff = 0.05)
dotplot(go2)
heatplot(go2,foldChange = genes_dfe_list)
...
dfe_lfc$cluster = cl_prot
genes_dfe_cluster = dfe_lfc %>% drop_na(ORF) %>% group_split(as.factor(cluster))
