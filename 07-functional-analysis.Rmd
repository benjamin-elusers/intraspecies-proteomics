...
UP    = df_dfe$ORF[ df_dfe$ratio_strains > 1 ]
DOWN  = df_dfe$ORF[ df_dfe$ratio_strains < -1 ]
BOTH  = df_dfe$ORF[ between(df_dfe$ratio_strains,c(-1,1)) ]
GRP = list(upregulated=UP,downregulated=DOWN,both=BOTH)


GO = gprofiler2::gost(GRP, organism = 'scerevisiae', significant = T, 
                 user_threshold = 0.01, custom_bg = genes_proteomics)
GG = gprofiler2::gostplot(GO,interactive = T)
pp <- publish_gostplot(GG, highlight_terms = GO$result,height = 12,width=14, filename=here::here('plot','GO-gprofiler2-up_down_both-pv5.pdf'))
...
gs_cc <- gseGO(geneList= genes_dfe_list, OrgDb = org.Sc.sgd.db, keyType = "ORF", 
              ont = "CC", minGSSize = 20, maxGSSize = 500, pvalueCutoff = 1e-2)
gs_mf <- gseGO(geneList= genes_dfe_list, OrgDb = org.Sc.sgd.db, keyType = "ORF", 
              ont = "MF", minGSSize = 20, maxGSSize = 500, pvalueCutoff = 1e-2)
gs_bp <- gseGO(geneList= genes_dfe_list, OrgDb = org.Sc.sgd.db, keyType = "ORF", 
              ont = "BP", minGSSize = 20, maxGSSize = 500, pvalueCutoff = 1e-2)

# gs_all <- gseGO(geneList= genes_dfe_list, OrgDb = org.Sc.sgd.db, keyType = "ORF", 
#               ont = "ALL", minGSSize = 20, maxGSSize = 500, pvalueCutoff = 1e-2) 

if( has_enriched(gs_mf) ){
  g1 = dotplot(gs_mf,color='p.adjust',
                 label_format=WRAPPING,font.size=FONT_SIZE,
                 title='GeneSet enrichment - Molecular Function') 
}
if( has_enriched(gs_bp) ){
  g2 = dotplot(gs_bp, color='p.adjust',
               label_format=WRAPPING,font.size=FONT_SIZE,
               title='GeneSet enrichment - Biological Process')
}
if( has_enriched(gs_cc) ){
  g3 =  dotplot(gs_cc,color='p.adjust',
                label_format=WRAPPING,font.size=FONT_SIZE,
                title='GeneSet enrichment - Cellular Component')
}

# gall =  dotplot(gs_all,color='p.adjust',
                # label_format=WRAPPING,font.size=FONT_SIZE,
                # title='GeneSet enrichment - All ontologies')
g1 | g2 | g3

library(gprofiler2)

gostres <- gost(query = names(genes_dfe_list), 
                organism = "scerevisiae", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
                user_threshold = 0.01, correction_method = "g_SCS", 
                domain_scope = "custom", custom_bg = genes_proteomics, 
...
gostres <- gost(query =genes_dfe_cluster, 
                organism = "scerevisiae", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
                user_threshold = 0.01, correction_method = "g_SCS", 
                domain_scope = "custom", custom_bg = genes_proteomics, 
                numeric_ns = "", sources = NULL, as_short_link = FALSE)
gostplot(gostres, capped = TRUE, interactive = F)
