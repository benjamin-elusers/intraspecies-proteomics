...
```{r dfe-gene }
# Without NA
volcano_data =  get_volcano_data(input_data=int_bpca, min_lfc=2, min_pval=0.01, topn = 50) %>%
                bind_rows() %>% as_tibble()
...
dfe = get_dfe(INPUT=int_bpca, WHICH='both',MIN_LFC=2, MIN_PVAL=0.01, TOPN = 20, 
              count_up=T, count_down=T)

gene_diffexp = unique(dfe$ID)
gene_sameexp = unique(setdiff(volcano_data$ID,dfe$ID) )

int_bpca = int_bpca %>% mutate(is_regulated = uniprot %in% gene_diffexp)

sc_abundance = load.abundance()
strain_exp= int_bpca %>%  group_by(uniprot) %>% mutate(mean_strains = mean(c_across(all_of(toupper(samples)))) )
