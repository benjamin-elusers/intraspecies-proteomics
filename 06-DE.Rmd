...
```{r dfe-gene }
# Without NA
volcano_data =  get_volcano_data(input_data=int_norm_nona, min_lfc=2, min_pval=0.01, topn = 100) %>%
                bind_rows() %>% as_tibble()
dfe_nona = subset(volcano_data,sig!='Non significant')
N_DFE_NONA = nrow(dfe_nona)
NPROT_DFE_NONA =  n_distinct(dfe_nona$ID)
R_NONA = N_DFE_NONA/NPROT_DFE_NONA %>% round
...
R_IMPUTE= N_DFE_IMPUTE/NPROT_DFE_IMPUTE %>% round
