...
pairwise_strains = pairwise_condition(all_strains,as_pair = T,rm.duplicated = T,rm.identical = T)
pairwise_all_strains = pairwise_condition(all_strains,as_pair = T,rm.duplicated = F,rm.identical = T)
...
DE_all = c(ARP3 = "P47117", CDC33 = "P07260", OYE2 = "Q03558", 
           PMI40 = "P29952", TIF3 = "P34167", DED1 = "P06634", 
           ILV5 = "P06168", NIP1 = "P32497",  PGK1 = "P00560", ERG13 = "P54839")
sc_annotation %>% filter(UNIPROT %in% DE_all) %>% DT::datatable()
pheatmap::pheatmap(int_bpca[DE_all,], labels_row = names(DE_all))
...
limma_all = compare_conditions(input = df_int_bpca, id_col = "uniprot",comparison = pairwise_all_strains,col_group = 1)
nde_all=sapply(pairwise_all_strains,
           function(x){ topTable(limma_all, coef=x, number = Inf, lfc = 2, p.value = 1e-5, adjust.method = 'BH') %>% nrow }
)

limma_p = compare_conditions(input = df_int_bpca, id_col = "uniprot",comparison = pairwise_strains,col_group = 1)
nde_p=sapply(pairwise_strains,
           function(x){ topTable(limma_p, coef=x, number = Inf, lfc = 2, p.value = 1e-5, adjust.method = 'BH') %>% nrow }
)

limma_one = compare_conditions(input = df_int_bpca, id_col = "uniprot",comparison = all_strains,col_group = 1)
nde_one=sapply(all_strains,
           function(x){ topTreat(limma_one, coef=x, number = Inf, lfc = 2, p.value = 1e-5, adjust.method = 'BH') %>% nrow }
)

sum(nde_one)
sum(nde_p)
topTreat(limma_one, coef = 1, number=Inf, lfc=2, p.value=1e-5)

volcano_imp =  get_volcano_data(input_data=df_int_bpca, 
                                min_lfc=CUTOFF_LFC, min_pval=CUTOFF_PV, 
                                topn = 50,combination = pairwise_all_strains) %>%
                bind_rows() %>% as_tibble() %>% 
                group_by(ID) %>%
...
pDE =ggplot(nprot_DE2) + 
  ...
  scale_fill_metro() +
  ylab('Number of Proteins')+ xlab('Strains') +
...
```{r dfe-gene, fig.path='plot/', fig.show='hold', warning=F, out.width='100%' }
...
dfe = get_dfe(INPUT=int_bpca, WHICH='both',MIN_LFC=CUTOFF_LFC, MIN_PVAL=CUTOFF_PV, TOPN = 20, 
      ...
      group_by(S1,ID,sig) %>% mutate(n_sig = n()) %>% 
...
```{r dfe-features, fig.path='plot/', fig.show='hold', warning=F, out.width='100%'}
# Without NA
...
sc_abundance = load.abundance() %>% 
                dplyr::rename_with(.cols=-orf, .fns = Pxx, px="cat_expression", s='.'), by=c('orf'))
strain_exp= df_int_bpca %>%  group_by(uniprot) %>%
  mutate(mean_strains = mean(c_across(all_of(toupper(samples)))) )

# evo_yeast %>% 
#   dplyr::rename_with(.cols=-c(orf,UNIPROT,GENENAME,SGD,OG), .fn=Pxx, px="cat_evolution")

DD = volcano_data %>% group_by(ID) %>% 
      summarize(cat_expression.var_lfc = var(EffectSize), 
                cat_expression.mean_lfc = mean(EffectSize)) %>%
      #left_join(, by=c('ID'='UNIPROT')) %>% 
      dplyr::left_join(sc_abundance) %>% 
