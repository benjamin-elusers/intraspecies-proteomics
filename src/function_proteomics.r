...
draw_heatmap_samples = function(mcor,df.group,col.group,k=2){
  ...
  #                    width = 14,height=12, border_color = NA, #cellwidth = 4, cellheight = 4,
  #                    filename =here::here('plot','correlation-heatmap-norm.intensities.pdf'))
...
draw_normalization_density = function(RAW = int_raw, INT=int_lfq, DESIGN = df.group){
  ...
  df_raw$norm_int_log2 = log2(df_raw$norm_int)
  library(geomtextpath)
  dens_plot = ggplot(df_norm,aes(x=norm_int)) +
    geom_density(data=subset(df_raw, strain!='AMH'), mapping=aes(x=norm_int_log2,group=strain),
                 col='black',show.legend = F,size=0.2,linetype='dashed') +
    #geom_textdensity(data=subset(df_raw, strain=='AMH'), mapping=aes(x=norm_int_log2,group=strain,label=type),
