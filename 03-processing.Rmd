...
```{r impute-na}
library(MsCoreUtils)
int_bpca = MsCoreUtils::impute_bpca(int_norm) %>% as_tibble() %>% add_column(uniprot = int_norm_ids$uniprot)
```
...
```{r int-strains}
long_int_norm = pivot_longer(int_bpca  , cols=-uniprot, values_to='int2use',
...
int_all = int_bpca  %>% column_to_rownames('uniprot') %>% as.data.frame()
