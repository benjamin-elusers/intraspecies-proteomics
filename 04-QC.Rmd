...
tot_int_norm = sum_intensities(int_norm)
...
bp_na = ggplot(count_NA_samples,aes(x=sample,label=sample)) + 
  ...
  geom_text(data=subset(count_NA_samples,!is_outlier),aes(y=0),angle=90,hjust=-0.1,size=2.5)+
  geom_text(data=subset(count_NA_samples,is_outlier),aes(y=0),angle=90,hjust=-0.1,fontface='bold',size=2.5)+
  ylab('# NAs') +
#  geom_hline(yintercept = mean_na_outliers,size=1) + 
  geom_hline(yintercept = mean_na,linetype=2,size=0.5) +
  scale_fill_metro() +
  scale_color_manual(values = c('TRUE'='black','FALSE'='white')) + 
  facet_wrap(~strain,scale='free_x',nrow = 2) +
  th_strip + 
...
```{r na-samples, echo=F, fig.cap='Number of quantified intensities across samples', out.width='100%'}
...
```{r boxplot-cv, echo=F, fig.cap='Coefficient of variations (%) across samples for each normalization method'}
...
```{r imp_vs_norm, fig.height=6, fig.width=12,out.width='100%'}
long_bpca = int_bpca %>% 
...
```{r delta_imp_norm, out.width='100%'}
d_exp = df_exp %>%
        group_by(strain) %>%  
...
COR_RANGE = range( cs_all[ upper.tri(COR_RANGE) ] )
